...
const CHARACTERS: CharacterSeed[] = [
  {
    ...
    portraitUrl: resolveCharacterPortrait("ethan-carter"),
  },
  {
    slug: "noah",
    displayName: "Noah",
    portraitUrl: resolveCharacterPortrait("noah"),
  },
  {
    slug: "robert",
    displayName: "Robert",
    portraitUrl: resolveCharacterPortrait("robert"),
  },
  {
    slug: "daniel-brooks",
    displayName: "Daniel Brooks",
    portraitUrl: resolveCharacterPortrait("daniel-brooks"),
  },
  {
    slug: "supervisor",
    displayName: "Ethan's Supervisor",
    portraitUrl: resolveCharacterPortrait("supervisor"),
  },
];
...
const EVIDENCE: EvidenceSeed[] = [
  {
    ...
    attachmentUrl: resolveEvidenceAttachment("toxicology-report"),
  },
  {
    slug: "wallet-ledger-photo",
    title: "Wallet Ledger",
    type: EvidenceType.WALLET_LEDGER,
    content:
      "Same source wallet, three separate buyers, all within a six-hour window.",
    attachmentUrl: resolveEvidenceAttachment("wallet-ledger-photo"),
  },
  {
    slug: "burner-phone-tower-logs",
    title: "Cell Tower Logs",
    type: EvidenceType.GPS_PIN,
    content:
      "Triangulated location data recovered from the burner phone's carrier records.",
    attachmentUrl: resolveEvidenceAttachment("burner-phone-tower-logs"),
  },
  {
    ...
    content:
      "Hidden in a desk drawer. Every call routes through a relay that leaves nothing behind.",
    attachmentUrl: resolveEvidenceAttachment("secure-encrypted-phone"),
  },
  {
    slug: "noahs-keychain",
    ...
    type: EvidenceType.OTHER,
    content:
      "A small, worn keychain. Not evidence in any case file — just the only thing Ethan took home.",
    attachmentUrl: resolveEvidenceAttachment("noahs-keychain"),
  },
];
...
export async function seedStory(): Promise<void> {
  ...
  for (const scene of SCENES) {
    ...
    const evidenceId = scene.evidenceSlug
      ? (evidenceIdBySlug.get(scene.evidenceSlug) ?? null)
      : null;
    if (scene.evidenceSlug && !evidenceId) {
      throw new Error(
        `[seed-story] Scene "${scene.slug}" references unknown evidence "${scene.evidenceSlug}".`,
      );
    }
    ...
    for (const line of scene.dialogueLines) {
      const characterId = line.characterSlug
        ? (characterIdBySlug.get(line.characterSlug) ?? null)
        : null;
  ...
  if (!overdoseChallenge) {
    throw new Error(
      '[seed-story] Unlock rules require challenge "the-overdose-report" to exist.',
    );
  }
